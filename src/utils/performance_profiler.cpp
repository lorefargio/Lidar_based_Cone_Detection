...
PerformanceProfiler::PerformanceProfiler(const std::string& algorithm_name) 
    ...
    frames_.reserve(5000); // Pre-allocate memory to avoid reallocation during runtime
}
...
void PerformanceProfiler::stopTimer(const std::string& phase) {
    ...
    if (timers_.find(phase) != timers_.end()) {
        ...
        if (phase == "ground_removal") current_frame_data_.ground_removal_ms = elapsed;
        else if (phase == "clustering") current_frame_data_.clustering_ms = elapsed;
        else if (phase == "estimation") current_frame_data_.estimation_ms = elapsed;
        else if (phase == "total") current_frame_data_.total_ms = elapsed;
    }
}
...
void PerformanceProfiler::saveToJSON(const std::string& filepath) {
    ...
    if (!out.is_open()) {
        std::cerr << "Errore nell'aprire il file JSON per il salvataggio: " << filepath << std::endl;
        return;
    }
